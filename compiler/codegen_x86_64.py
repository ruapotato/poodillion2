...
class X86_64CodeGen:
    ...
    def gen_expression(self, expr: ASTNode) -> str:
        ...
        if isinstance(expr, IndexExpr):
            ...
            arr_type = self.get_expr_type(expr.base)
            elem_size = 8
            elem_type = None
        ...
        if isinstance(expr, AddrOfExpr):
            ...
            if isinstance(expr.expr, Identifier):
                ...
                self.emit("pop rcx")   # Index in RCX
                # Get element type and size
                arr_type = self.get_expr_type(expr.expr.base)
                elem_size = 8
                if isinstance(arr_type, ArrayType):
                    elem_size = self.type_size(arr_type.element_type)
                elif isinstance(arr_type, PointerType):
    ...
    def gen_statement(self, stmt: ASTNode):
        ...
        if isinstance(stmt, VarDecl):
            ...
            if isinstance(stmt.target, Identifier):
                ...
                arr_type = self.get_expr_type(stmt.target.base)
                elem_size = 8
                if isinstance(arr_type, ArrayType):
                    elem_size = self.type_size(arr_type.element_type)
                elif isinstance(arr_type, PointerType):
    ...
    def gen_procedure(self, proc: ProcDecl):
        ...
        # Default return value (Python auto-return None)
        if proc.return_type:
    ...
    def generate(self, program: Program) -> str:
        ...
        # Header
        asm_lines.append("; Generated by Brainhair x86_64 compiler")
        asm_lines.append("[BITS 64]")
        asm_lines.append("")
        ...
        for decl in program.declarations:
            if isinstance(decl, ExternDecl):
                asm_lines.append(f"extern {decl.name}")

        # Global exports
        if self.kernel_mode:
            asm_lines.append("")
            asm_lines.append("; Kernel exports")
            asm_lines.append("global brainhair_kernel_main")
            # Add other kernel exports here as needed
        else:
            asm_lines.append("")
            asm_lines.append("; Userland exports")
        ...
        if self.strings:
            ...
            for label, value in self.strings.items():
                ...
                asm_lines.append(f'{label}: db {", ".join(bytes_list)}')
        ...
        if bss_vars:
            ...
            for name, size in bss_vars:
                asm_lines.append(f"{name}: resb {size}")
