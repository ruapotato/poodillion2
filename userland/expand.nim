...
proc main() =
  ...
  while i < argc:
    ...
    if strcmp(arg, cast[ptr uint8]("-t")) == 0:
      i = i + 1
      if i >= argc:
        print_err(cast[ptr uint8]("expand: -t requires tabstop value\n"))
        discard syscall1(SYS_exit, 1)
      var tab_arg: ptr uint8 = get_argv(i)
      ...
      if arg[0] != cast[uint8](45):  # not a flag
        ...
        if input_fd < 0:
          print_err(cast[ptr uint8]("expand: cannot open file\n"))
          discard syscall1(SYS_exit, 1)
    i = i + 1
  ...
  while running != 0:
    ...
    while j < n:
      ...
      if ch == cast[uint8](9):  # tab
        ...
        var spaces_needed: int32 = tabstop - (col % tabstop)
        var k: int32 = 0
