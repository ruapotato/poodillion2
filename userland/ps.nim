...
proc write_schema_header() =
  ...
  # Version: 1
  var version: uint8 = 1
  discard syscall3(SYS_write, STDOUT, cast[int32](cast[ptr uint8](version)), 1)

  # Field count: 3 (pid, memory, name_len for now)
  var field_count: uint8 = 3
  discard syscall3(SYS_write, STDOUT, cast[int32](cast[ptr uint8](field_count)), 1)

  # Record size: 12 bytes (4 + 8 + 0)
  var rec_size: uint16 = 12
  discard syscall3(SYS_write, STDOUT, cast[int32](cast[ptr uint16](rec_size)), 2)

proc write_process_record(pid: int32, mem: int32) =
  # For now, simple demo: just write pid and placeholder memory
  discard syscall3(SYS_write, STDOUT, cast[int32](cast[ptr int32](pid)), 4)
  discard syscall3(SYS_write, STDOUT, cast[int32](cast[ptr int32](mem)), 4)

  # Placeholder for name length
  var name_len: int32 = 0
  discard syscall3(SYS_write, STDOUT, cast[int32](cast[ptr int32](name_len)), 4)
...
proc main() =
  ...
  var count: int32 = 3
  discard syscall3(SYS_write, STDOUT, cast[int32](cast[ptr int32](count)), 4)
