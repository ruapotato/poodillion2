...
proc main() =
  ...
  print_err(cast[ptr uint8]("killall: Searching for process: sleep\n"))
  ...
  if found_count == 0:
    print_err(cast[ptr uint8]("killall: no process found\n"))
    discard syscall1(SYS_exit, 1)

  print_err(cast[ptr uint8]("killall: Killing "))
  ...
  while i < found_count:
    ...
    if result == 0:
      print_err(cast[ptr uint8]("Killed PID "))
      print_int(pid)
      discard syscall3(SYS_write, STDERR, cast[int32]("\n"), 1)
      killed = killed + 1
    else:
      print_err(cast[ptr uint8]("Failed to kill PID "))
      print_int(pid)
      discard syscall3(SYS_write, STDERR, cast[int32]("\n"), 1)
  ...
  if killed > 0:
    print_err(cast[ptr uint8]("killall: Successfully killed "))
    print_int(killed)
    print_err(cast[ptr uint8](" process(es)\n"))
    discard syscall1(SYS_exit, 0)

  print_err(cast[ptr uint8]("killall: No processes were killed\n"))
  discard syscall1(SYS_exit, 1)
