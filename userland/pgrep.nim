...
# Usage: pgrep PATTERN
#        pgrep -l PATTERN (show process name)
# Searches /proc/*/comm for matching processes
...
# Simple substring search (checks if needle is in haystack)
...
proc main() =
  ...
  # Default: show PIDs only
  var show_name: int32 = 0

  # For demonstration, search for pattern "bash"
  # In a real implementation, this would come from argv
  # To support -l flag, we'd check argv[1] for "-l"
  pattern[0] = cast[uint8](98)   # b
  pattern[1] = cast[uint8](97)   # a
  pattern[2] = cast[uint8](115)  # s
  pattern[3] = cast[uint8](104)  # h
  pattern[4] = cast[uint8](0)
  ...
  if proc_fd < 0:
    print_err(cast[ptr uint8]("pgrep: cannot open /proc\n"))
